...
def hough_algorithm():
    # This is an array
    hough_transform_output = cv.HoughLinesP(roi, rho=2, theta=3.642 / 180, threshold=75, minLineLength=40,
                                            maxLineGap=1)
...
while (cap.isOpened()):
    ...
    lines = cv.HoughLinesP(cropped_image, rho=1, theta=3.642 / 180, threshold=120, minLineLength=15,
                           maxLineGap=50)  # (roi, rho=2, theta=3.642 / 180, threshold=100, minLineLength=40, maxLineGap=5)
    averaged_lines = lane_line_average(frame, lines)
    line_image = display(frame, averaged_lines)
    final = cv.addWeighted(frame, 0.8, line_image, 1, 2)
